$(function() {
	var oldBillTotal = "";
    var piClass = "pi label label-success";
    var palindromeClass = "palindrome label label-primary";
    var countingClass = "counting label label-secondary";
	...
	function update() {
		...
		if (isPi(totalPenniesStr)) {
			totalPenniesSpan.addClass(piClass);
		}
		if (isPalindrome(totalPenniesStr)) {
			totalPenniesSpan.addClass(palindromeClass);
		}
		if (isCounting(totalPenniesStr)) {
			totalPenniesSpan.addClass(countingClass);
		}
		...
		for (var i = minTipPennies; i <= maxTipPennies; i++) {
			...
			if (isPi(totalAmtPenniesStr)) {
				totalAmtPenniesSpan.addClass(piClass);
				interesting = true;
			}
			if (isPi(iStr)) {
				tipSpan.addClass(piClass);
				interesting = true;
			}
			if (isPalindrome(totalAmtPenniesStr)) {
				totalAmtPenniesSpan.addClass(palindromeClass);
				interesting = true;
			}
			if (isPalindrome(iStr)) {
				tipSpan.addClass(palindromeClass);
				interesting = true;
			}
			if (isCounting(totalAmtPenniesStr)) {
				totalAmtPenniesSpan.addClass(countingClass);
				interesting = true;
			}
			if (isCounting(iStr)) {
				tipSpan.addClass(countingClass);
				interesting = true;
			}
